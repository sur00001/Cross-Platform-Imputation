...
summarize_res = function(y_pred,y_test){
  mse = sapply_pb(1:ncol(y_test),function(i) mean((y_pred[,i]-y_test[,i])^2,na.rm=TRUE))
  R = sapply_pb(1:ncol(y_test),function(i) cor(y_pred[,i],y_test[,i],use="pairwise.complete.obs"))
  R2 = R*R
  tab = c(mse=mean(mse,na.rm=TRUE),R2=mean(R2,na.rm=TRUE))
  return(list(mse=mse,R2=R2,tab=tab))
}
