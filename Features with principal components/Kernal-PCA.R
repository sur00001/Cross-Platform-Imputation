...
#Linearly scale data for K-mean clusters for kernal with PCs
pc.ker.cluster=function(ytrain,xtrain,xtest,ytest){
  for (i in ncol(xtrain)){
    xtrain[,i]=(xtrain[,i]-mean(xtrain[,i]))/(max(xtrain[,i])-min(xtrain[,i]))
    xtest[,i]=(xtest[,i]-mean(xtest[,i]))/(max(xtest[,i])-min(xtest[,i]))
  }
  for (i in ncol(ytrain)){
  ytrain[,i]=(ytrain[,i]-mean(ytrain[,i]))/(max(ytrain[,i])-min(ytrain[,i]))
  ytest[,i]=(ytest[,i]-mean(ytest[,i]))/(max(ytest[,i])-min(ytest[,i]))
  }
  ypred = matrix(NA,nrow=nrow(xtest),ncol=ncol(ytest))
  rbfker <- rbfdot(sigma = .0014)
  system.time(ypred <- kernel_pred(xtrain,ytrain,xtest,ker=rbfker,alpha=0.99))
  ker_res = summarize_res(ypred,ytest)
  ker_res}
####################### Kernal Result ###########################
#Tune sigma for kernal (have to run contents of function)
sigest(t(x_train))

#Tune sigma
#Trying sigma values within sigest range that maximize mean R^2
s = r = r.new = 0 #Initialize parameters
for (i in seq(.0012,.0026,.00001)){
  rbfker <- rbfdot(sigma = i)
  y_pred <- kernel_pred(x_train,y_train,x_test,ker=rbfker,alpha=0.99)
  ker_res = summarize_res(y_pred,y_test)
  r.new = mean(ker_res$R2)
  if (r.new > r){
    r = r.new
    s=i
  }
...
PC22.ker = summarize_res(y_pred,y_test); PC22.ker$tab 


##### 200 random sites #####
start_time=Sys.time()
pca_ker.res=kernal(chromo=22,PCA=3,pca.preds=FALSE,sig=.0026)
end_time=Sys.time()
end_time-start_time
